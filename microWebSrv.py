...
class MicroWebSrv:
    ...
    def _startThread(func, args=()):
        try:
            ...
            try:
                ...
                _mwsrv_thread_id = 0
            try:
                start_new_thread('MWSRV_THREAD_%s' %
                                 _mwsrv_thread_id, func, args)
            except:
                return False
        return True
    ...
    def __init__(self,
        ...
        for route, method, func in routeHandlers:
            ...
            self._routeHandlers.append(MicroWebSrvRoute(
                route, method, func, routeArgNames, routeRegex))
    ...
    class _client:
        ...
        def _parseHeader(self, response):
            while True:
                ...
                if len(elements) == 2:
                    self._headers[elements[0].strip().lower()
                                  ] = elements[1].strip()
    ...
    class _response:
            ...
            try:
                ...
                if size > 0:
                    with open(filepath, 'rb') as file:
                        self._writeBeforeContent(
                            200, headers, contentType, None, size)
                        try:
                            buf = bytearray(1024)
                            while size > 0:
                                x = file.readinto(buf)
        ...
        def WriteResponseError(self, code):
            responseCode = self._responseCodes.get(
                code, ('Unknown reason', ''))
